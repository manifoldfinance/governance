...
contract GovernanceMothership is Ownable, BalanceAccounting {
    ...
    IERC20 public immutable foldToken;

    EnumerableSet.AddressSet private _modules;

    constructor(IERC20 _foldToken) public {
        foldToken = _foldToken;
    }

    function name() external pure returns(string memory) {
        return "1fold Token (Staked)";
    }
    ...
    function stake(uint256 amount) external {
        ...
        foldToken.transferFrom(msg.sender, address(this), amount);
    ...
    function unstake(uint256 amount) external {
        ...
        _notifyFor(msg.sender, balanceOf(msg.sender));
        foldToken.transfer(msg.sender, amount);
